  ...
  <section class="my-16 px-6 max-w-6xl mx-auto text-center">
    ...
    <div class="relative overflow-hidden">
      <div class="flex transition-transform duration-700 ease-in-out" :style="{ transform: `translateX(-${currentIndex * 100}%)` }">
        <div v-for="(group, i) in techGroups" :key="i" class="min-w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 px-4">
          <div v-for="tech in group" :key="tech.name" class="bg-white/80 backdrop-blur-lg shadow-md rounded-xl p-6 hover:scale-105 hover:shadow-xl transition">
            ...
            <p class="text-gray-800 font-medium text-sm">{{ tech.name }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="absolute top-1/2 left-0 transform -translate-y-1/2">
        <button @click="prev" class="bg-gray-200 p-4 rounded-full hover:bg-blue-500 hover:text-white">
          <
        </button>
      </div>
      <div class="absolute top-1/2 right-0 transform -translate-y-1/2">
        <button @click="next" class="bg-gray-200 p-4 rounded-full hover:bg-blue-500 hover:text-white">
          >
        </button>
      </div>
    </div>
...
function prev() {
  currentIndex.value =
    (currentIndex.value - 1 + techGroups.value.length) % techGroups.value.length;
}
...
const technologies = [
    ...
    { name: 'GitHub', logo: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg' },
    { name: 'Node.js', logo: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg' },
    { name: 'REST API', logo: 'https://cdn-icons-png.flaticon.com/512/1006/1006771.png' } // API icon no oficial pero visual
];